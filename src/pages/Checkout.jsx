...
import { useCart } from '../hooks/useCart';             // Hook pour récupérer le panier
import { useCheckout } from '../hooks/useCheckout';     // Hook pour gérer la commande
import { CreditCard, Building, Package, CheckCircle } from 'lucide-react'; // Icônes

/**
 * Composant : Checkout
 * --------------------
 * Page de paiement qui gère :
 * - Affichage des détails du panier
 * - Formulaires de facturation et livraison
...
const Checkout = () => {
  ...
  // --- Etat local pour savoir si l'adresse de livraison est différente ---
  const [shipToDifferent, setShipToDifferent] = useState(true);

  // --- Données du formulaire ---
  const [formData, setFormData] = useState({
    billing: { civility: 'Mr', firstName: '', lastName: '', companyName: '', street: '', zipCode: '', city: '', county: '', apartment: '', email: '', phone: '' },
    shipping: { civility: 'Mr', firstName: '', lastName: '', companyName: '', street: '', zipCode: '', city: '', county: '' },
    orderNotes: '',
    paymentMethod: 'bacs',
  });
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* --- Partie gauche : Adresses et notes --- */}
            <div className="space-y-8">
              {/* Billing Details */}
              <div className="bg-white p-6 rounded-lg shadow-sm">
                <h2 className="text-2xl font-bold text-[#5a88ca] uppercase mb-6 tracking-wider">Billing Details</h2>
                <AddressForm
                  prefix="billing"
            ...
            <div className="space-y-6">
              {/* Your Order */}
              <div className="bg-white p-6 rounded-lg shadow-sm">
                <h2 className="text-2xl font-bold text-[#5a88ca] uppercase mb-6 tracking-wider">Your Order</h2>
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {cart.items.map((item) => (
                      <tr key={item.id} className="border-b">
                        <td className="py-3">{item.name} <strong className="text-gray-500">× {item.qty}</strong></td>
                  ...
                  <tfoot>
                    ...
                    <tr>
                      ...
                      <td className="text-right py-2 text-lg font-bold text-green-600">€{cart.total.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              {/* Payment Methods */}
              <div className="bg-white p-6 rounded-lg shadow-sm">
                <h2 className="text-2xl font-bold text-[#5a88ca] uppercase mb-6 tracking-wider">Payment Method</h2>
              ...
              >
                {isLoading ? 'Placing Order...' : <><CheckCircle className="w-5 h-5" /> Place Order</>}
              </button>
 ...
 * ----------------------
 * Formulaire pour billing / shipping.
 */
const AddressForm = ({ prefix, data, onChange }) => (
  <div className="space-y-4">
    {/* Civility */}
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">Civility <span className="text-red-500">*</span></label>
      <select value={data.civility} onChange={(e) => onChange('civility', e)} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
...
const Input = ({ label, type = 'text', required, ...props }) => (
  <div>
    ...
    <input type={type} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent" required={required} {...props} />
  </div>
);

/**
 * Composant PaymentOption
 */
const PaymentOption = ({ id, label, icon, checked, onChange, description }) => (
  <label className="flex items-start gap-3 cursor-pointer p-3 rounded border hover:bg-gray-50 transition">
    <input type="radio" name="payment_method" value={id} checked={checked} onChange={onChange} className="mt-1" />
    <div className="flex-1">
      <div className="flex items-center gap-2 font-medium">{icon}<span>{label}</span></div>
